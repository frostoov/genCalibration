	...
	//BEGIN
		int max1 = 0, max2 = 0, max3 = 0, max4 = 0;
			...
			for (j=0; j<Aperture; j++)
			{
				...
				if (max1 < DATA.pls[0][j])
					max1 = DATA.pls[0][j];
				if (max2 < DATA.pls[0][j + 1024])
					max2 = DATA.pls[0][j + 1024];
				if (max3 < DATA.pls[0][j + 2048])
					max3 = DATA.pls[0][j + 2048];
				if (max4 < DATA.pls[0][j + 3072])
					max4 = DATA.pls[0][j + 3072];
				fprintf(fsub,"\n");
			}
//			printf("%d   %d    %d    %d\n", max1, max2, max3, max4);
		FILE* mainFile;
